...
| Bytes y bits del estado en tiempo real | `grep -n "CMD_ESTADO_PAPEL\|CMD_ESTADO_IMPRESORA\|_MASCARA_FIJA_ESTADO\|BITS_SIN_PAPEL" ruleta/escpos.py` → **re-medido el 2026-09-15 tras la Fase 4a: 58-62 y 65-67** (constantes), más **74-76** (los bits de `DLE EOT 2`, nuevos) y **81-85** (los topes de la lectura fresca); usos en **120-122** (`es_estado_valido`), **125-169** (`leer_estado_fresco`, nueva), **172-213** (`verificar_estado`) y **746-763** (`consultar_papel`, con las tres consultas en **759-761**) | Son los valores exactos de los goldens de (b): `0x93`/`0x12`, `0x60`, `0x0C`, `0x08`. Desde 2b los tres últimos tienen nombre (`BITS_SIN_PAPEL`, `BITS_POCO_PAPEL`, `BIT_FUERA_DE_LINEA`) y los comparte `ruleta/__main__.py` (**359-368**, re-medido); la Fase 4a añadió `CMD_ESTADO_CAUSA`, `BIT_TAPA_ABIERTA`, `BIT_FIN_DE_PAPEL` y `BIT_ERROR_IMPRESORA`, y borró `_MAX_BYTES_BASURA` |
...
| **Aviso de «sin papel» por USB** | §0-bis H5 y cambio (b) | **Corregido el 2026-09-15 (Fase 4a): lo de «funcionando en hardware real» era falso.** Lo único que probó el 2026-09-11 es que **el nodo acepta la escritura de la consulta**; el byte que se leía era el de la **pregunta anterior**, y aquel aviso de «poco papel» era **falso** (**F-190**). El caso extremo **ya se probó, el 2026-09-15, y falló**: con el rollo agotado el kiosco dio por impreso un boleto que no salió (**F-091**, **F-250**). Lo arregla la **Fase 4a** (`docs/planes/fase-4a-papel.md`): lectura fresca y `DLE EOT 2` |
...
| **Tener un rollo de repuesto** | ficha F-190 | **Corregido el 2026-09-15 (Fase 4a):** el aviso de «poco papel» del 2026-09-11 al 2026-09-15 se midió **falso** (byte rezagado; el último salió en la jugada sin papel de las 13:29 del 2026-09-15) y el rollo que está puesto es **nuevo** desde el 2026-09-15. Lo que hace falta antes del evento es **un rollo de repuesto junto a la Pi** |
